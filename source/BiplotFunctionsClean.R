...
{  dim.biplot <- dim.biplot[1]
   ...
   ax <- do.call("biplot.ax.control", c(num.vars, list(var.names), ax))
   if (ax$type == "prediction") 
     axes.direction <- solve(diag(diag(t(Mrr.all) %*% Mrr.all)),tol=1e-33) %*% t(Mrr.all) %*% rotate.mat %*% reflect.mat
   else 
...
CVA_H<-
             ...
             ...)
   {
      ...
      names(zlist) <- dimnames(X)[[2]]
      list (zlist = zlist , H = Hmat.cent,Zmat=Z,RSS=RSS,diff=ep, Zmeans=Z.means.mat)
   }
